...
export async function uploadToCloudinary(imageUrl: string): Promise<string> {
  ...
  if (!cloudName || !uploadPreset) {
    throw new Error("CLOUDINARY_CLOUD_NAME and CLOUDINARY_UPLOAD_PRESET env vars are required to upload images");
  }

  const body = new URLSearchParams({ file: imageUrl, upload_preset: uploadPreset });
