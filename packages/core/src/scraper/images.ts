...
export async function uploadProductImages(
  ...
  products.forEach((product, productIndex) => {
    product.images.forEach((url, imageIndex) =>
      refs.push({ productIndex, imageIndex, url }),
    );
  });

  const uploaded = await runInBatches(
    refs,
  ...
  for (const ref of uploaded)
    result[ref.productIndex].images[ref.imageIndex] = ref.url;
  return result;
}
